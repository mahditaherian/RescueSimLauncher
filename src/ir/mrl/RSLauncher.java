...
/**
 * Created by Rescue Agent on 6/12/15.
 */


public class RSLauncher extends JFrame implements WindowListener {
    private static final Logger logger = Logger.getLogger(RSLauncher.class.getName());
        ...
        try {
            ...
            while (p != null) {
                ...
                if (param[0].equalsIgnoreCase(defMapAddressString)) {
                    ...
                    logger.log(Level.WARNING, "data error in mrlKernel.config");
                }
                p = br.readLine();
            }
            br.close();
...
//        progLabel.setText("MRL - RSL kernel and code launcher (2013)");
        ...
        preComputeCheckBox.setText("Pre-Compute Run");
        ...
        loadKernelButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                if (getMapAddress(false) != null) {
                    ...
                    JOptionPane.showMessageDialog(RSLauncher.this,
                ...
                if (!saveFile.exists()) {
                    ...
                    if (!parent.exists()) {
                        if (!saveFile.getParentFile().mkdirs()) {
                            logger.log(Level.WARNING, "Couldn't create file " + saveFile.getPath());
                        }
                    }
...
//                ComponentLauncher launcher = LaunchMRL.launchAgents(agentArgs);
                ...
                //logger.log(Level.WARNING,e.getMessage());
                logger.log(Level.INFO, "failed: " + e.getMessage());
            }
        }
    }
        ...
        try {
            ...
            for (String arg : args) {
                if (arg.equalsIgnoreCase(LaunchKernel.NO_GUI)) {
                    ...
                    logger.log(Level.WARNING, "Unrecognised option: " + arg);
                }
            }
            // Process jar files
            ...
            for (String next : config.getArrayValue(Constants.MESSAGE_FACTORY_KEY, "")) {
                ...
                if (factory != null) {
                    ...
                    logger.log(Level.INFO, "Registered local message factory: " + next);
                }
            }
            for (String next : config.getArrayValue(Constants.ENTITY_FACTORY_KEY, "")) {
                ...
                if (factory != null) {
                    ...
                    logger.log(Level.INFO, "Registered local entity factory: " + next);
                }
            }
            for (String next : config.getArrayValue(Constants.PROPERTY_FACTORY_KEY, "")) {
                ...
                if (factory != null) {
                    ...
                    logger.log(Level.INFO, "Registered local property factory: " + next);
                }
            }
            ...
            if (kernelInfo == null) {
                logger.log(Level.WARNING, "kernelInfo is null");
                return;
            }
            KernelGUI gui = null;
            if (showGUI) {
            ...
            logger.log(Level.SEVERE, "Couldn't start kernel", e);
        } catch (KernelException e) {
            logger.log(Level.SEVERE, "Couldn't start kernel", e);
        } catch (IOException e) {
            logger.log(Level.SEVERE, "Couldn't start kernel", e);
        } catch (LogException e) {
            logger.log(Level.SEVERE, "Couldn't write log", e);
        } catch (InterruptedException e) {
            logger.log(Level.SEVERE, "Kernel interrupted");
        ...
        EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    RSLauncher rsLauncher = new RSLauncher();
                    rsLauncher.setFont(new Font("Vani", Font.BOLD, 12)); // NOI18N
                    rsLauncher.setTitle("MRL Rescue Simulation Launcher");
                    ImageIcon imageIcon = new ImageIcon("assets/rslogo.png");
                    rsLauncher.setIconImage(imageIcon.getImage());
                    rsLauncher.setExtendedState(MAXIMIZED_BOTH);
                    rsLauncher.setVisible(true);
                } catch (Exception ex) {
        ...
        try {
            if (!mrlKernelConfig.exists()) {
                if (!mrlKernelConfig.createNewFile()) {
                    logger.log(Level.WARNING, "Couldn't create file " + mrlKernelConfig.getPath());
                }
            }
